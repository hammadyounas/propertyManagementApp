...
const SyncfusionDocEditor = dynamic(
  async () => {
    ...
    const { DocumentEditorContainerComponent, Toolbar, Inject, Print } = await import(
...
const TemplateEditor = ({
  ...
  const docIdStampingInProgress = useRef(false)
  const docIdStampedRef = useRef(null)
  ...
  const ensureDocumentIdStamp = useCallback(() => {
    if (
      !documentId ||
      !docEditorRef.current?.documentEditor ||
      docIdStampedRef.current === documentId ||
      docIdStampingInProgress.current // Prevent concurrent calls
    ) {
      return
    }

    docIdStampingInProgress.current = true // Mark as in-progress

    try {
      const editorInstance = docEditorRef.current.documentEditor
      const editor = editorInstance?.editor
      const selection = editorInstance?.selection
      const searchModule = editorInstance?.searchModule

      if (!editor || !selection) return

      const stampText = `Doc ID: ${documentId}`
      const docText = editor.getText ? editor.getText() : ""

      if (docText.includes(stampText)) {
        docIdStampedRef.current = documentId
        docIdStampingInProgress.current = false // Mark as done
        return
      }
      ...
      const clearSearchHighlight = () => {
        try {
          ...
          console.warn("Failed clearing search highlight", err)
        }
      }

      const removeExistingStamp = (textToRemove) => {
        if (!searchModule || !textToRemove) return
        try {
          selection.goToPage(1)
          selection.moveToParagraphStart()
          clearSearchHighlight()
          searchModule.find(textToRemove)
          if (selection.text && selection.text.includes(textToRemove.replace("Doc ID:", "").trim())) {
      ...
      if (docIdStampedRef.current && docIdStampedRef.current !== documentId) {
        removeExistingStamp(`Doc ID: ${docIdStampedRef.current}`)
        docIdStampedRef.current = null
      }
      ...
      if (docText.includes("Doc ID:")) {
        removeExistingStamp("Doc ID:")
      }

      const moveToFirstContentParagraph = () => {
        try {
          selection.goToPage(1)
          selection.moveToParagraphStart()
          for (let i = 0; i < 30; i += 1) {
            selection.selectParagraph()
            const paragraphText = (selection.text || "").replace(/\s+/g, " ").trim()
            if (paragraphText && !paragraphText.toLowerCase().startsWith("doc id:")) {
              selection.moveToParagraphStart()
              return true
            }
            selection.moveDown()
          ...
          console.warn("Failed to navigate to first content paragraph", err)
        }
        return false
      }
      ...
      const insertStampParagraph = () => {
        // Set font properties
        selection.characterFormat.fontSize = 10 // Smaller font size for Doc ID
        selection.characterFormat.fontFamily = "Arial"
        selection.characterFormat.bold = true // Make it bold
        selection.paragraphFormat.textAlignment = "Left"
        selection.characterFormat.marginRight = 100
        // Set text color to gray (RGB: 128, 128, 128)
        selection.characterFormat.fontColor = "#808080"

        editor.insertText(stampText)
        editor.insertBreak(1)

        // Reset formatting after Doc ID for normal content
        selection.moveToParagraphStart()
        selection.characterFormat.fontSize = 11 // Reset to normal size
        selection.characterFormat.bold = false
        selection.characterFormat.fontColor = "#000000" // Black text
      }

      const insertHeadingSpacing = () => {
        selection.moveDown()
        selection.moveToParagraphStart()
      ...
      try {
        if (moveToFirstContentParagraph()) {
          insertStampParagraph()
          inserted = true
          insertHeadingSpacing()
        }
      } catch (err) {
        console.error("Failed to insert Doc ID", err)
      } finally {
        clearSearchHighlight()
        restoreSelection()
        if (inserted) {
          docIdStampedRef.current = documentId
          console.log("Document ID stamped inside document content.")
        }
        docIdStampingInProgress.current = false // Mark as done
      }
    } catch (error) {
      console.error("Failed to stamp Document ID:", error)
      docIdStampingInProgress.current = false // Mark as done
    }
  }, [documentId])
  ...
  useEffect(() => {
          ...
          switch (format) {
            case "sfdt":
              ...
              try {
                ...
                if (isAbbreviated) {
                  console.error(
                    "❌ CORRUPTED DOCUMENT: This document uses abbreviated SFDT format (optimizeSfdt: false)",
                  )
                  ...
                  const errorMessage =
                    `❌ DOCUMENT CORRUPTED - CANNOT LOAD\n\n` +
    ...
    return () => {}
  }, [mounted, editorValue, ensureDocumentIdStamp])

  // This prevents the double-stamping issue by reducing the number of calls
  useEffect(() => {
    if (!mounted || !documentId || !editorCreated || !contentReady) return

    docIdStampedRef.current = null

    const timeoutId = setTimeout(() => {
      ensureDocumentIdStamp()
    }, 400)

    return () => clearTimeout(timeoutId)
  }, [documentId, mounted, editorCreated, contentReady, ensureDocumentIdStamp])
  ...
  const handleDownload = async () => {
    try {
      const editor = docEditorRef.current?.documentEditor
      if (!editor) return
      const fileName = documentId ? `Doc ID ${documentId}` : docTitle ? docTitle : "Document"
      editor.documentName = fileName
      editor.print()
      if (onDownload) {
        await Promise.resolve(onDownload(fileName))
      }
    } catch (error) {
  ...
  useEffect(() => {
    ...
    // Define the attempts for adding buttons
    const attempts = [50, 150, 300, 600, 1000] // Delays in milliseconds
    ...
    const addButtons = () => {
      ...
      console.log("Adding Download and Import buttons to toolbar...")
      ...
      printBtn.title = "Download PDF"
      printBtn.style.cssText = "min-width: 80px; margin: 2px 4px;"
      ...
      const findBtn = Array.from(itemsEl.querySelectorAll(".e-tbar-btn")).find((n) =>
        (n.getAttribute("title") || "").toLowerCase().includes("find"),
      )
