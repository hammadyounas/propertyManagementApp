            ...
            <table className="min-w-full divide-y divide-slate-100 table-fixed dark:divide-slate-700">
              ...
              <tbody className="bg-white divide-y divide-slate-100 dark:bg-slate-800 dark:divide-slate-700">
                {rows.map((row, i) => (
                  ...
                  >
                    <td className="table-td">{row.invoice_number}</td>
                    ...
                    <td className="table-td">{row.client_address}</td>
                    <td className="table-td">{row.responsible_broker}</td>
                    <td className="table-td">{row.notary_date}</td>
                    <td className="table-td">{row.commissions_payable}</td>
                    <td className="table-td">
                      <span className="block w-full">
                        <span
                          className={`inline-block px-3 min-w-[90px] text-center mx-auto py-1 rounded-[999px] bg-opacity-25
                          ${row.status === "partially paid" && "text-blue-600 bg-blue-200"}
                          ${row.status === "refunded" && "text-purple-600 bg-purple-200"}
                          ${row.status === "cancelled" && "text-gray-600 bg-gray-200"}
                          ${row.tatus === "overdue" && "text-orange-600 bg-orange-200"}
                          ${row.status === "paid" && "text-teal-600 bg-teal-200"}
                          ${row.status === "sent" && "text-indigo-600 bg-indigo-200"}
