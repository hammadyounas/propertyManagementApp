...
export default function Templates2ListUI({
  templates,
  onEdit,
  onDelete,
  onDuplicate,
          ...
          <div className="w-full flex items-center max-sm:justify-end gap-4 whitespace-nowrap text-sm">              
            {/* <span className="">
              <Button
                text="Add Template"
                icon="heroicons:plus"
                onClick={() => router.push("/templates-2/create")}
                  ...
                  templates?.map((template, i) => (
                    ...
                    >
                      ...
                      <td className="table-td sm:p-4 p-2 text-center">
                        <div className="flex items-center justify-center">
                          <span className="text-primary-default font-bold cursor-pointer">
                            {template.createdBy || 'Polina'}
                          </span>
                        </div>
                      </td>
                      <td className="table-td sm:p-4 p-2">
                        {dateFormat(template.createdAt || template.modifiedAt)}
                      </td>
                      <td className="table-td sm:p-4 p-2">
                        <div className="flex justify-center">
                          <Icon
                            onClick={() => onEdit(template.fileName)}
                          ...
                          <div className="relative group inline-block">
                            <Icon
                              onClick={() => onDelete(template.fileName)}
